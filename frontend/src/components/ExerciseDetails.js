...
export function ExerciseDetails({ exercise, onBack }) {
    return (
        <Card className="p-6">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-2xl font-bold">{exercise.name}</h2>
                <Button onClick={onBack} variant="outline">Wróć do ćwiczeń</Button>
            </div>
            <div className="flex space-x-4 mb-4">
                <Button variant="outline">Zaawansowane</Button>
                <Button variant="secondary">Średni</Button>
            ...
            {exercise.videoUrl ? (
                <div className="aspect-w-16 aspect-h-9 mb-4">
                    <video src={exercise.videoUrl} controls className="rounded-lg w-full h-full object-cover" />
                </div>
            ) : (
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <p className="bg-gray-100 p-4 rounded-lg">{exercise.instructions}</p>
                </div>
                <div>
                    <h3 className="font-bold mb-2">Wzbogacenie</h3>
                    <p className="bg-gray-100 p-4 rounded-lg">{exercise.enrichment}</p>
                </div>
            </div>
        </Card>
    )
